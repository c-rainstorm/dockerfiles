...
if [ `docker images ${DOCKER_REPOSITORY_NAMESPACE}/oracle-jdk:1.8.0_191 -q | wc -l` -eq 0 ]; then
    echo "Local ${DOCKER_REPOSITORY_NAMESPACE}/oracle-jdk:1.8.0_191 not found, try to build"
    echo "Starting build ${DOCKER_REPOSITORY_NAMESPACE}/oracle-jdk:1.8.0_191"
    cd ${JDK_DIR} && ./build.sh --build-arg HTTPS_PROXY=https://host.docker.internal:1087 && cd ${PWD}
    echo "done"
fi

echo "Starting build rocketmq base image"
docker build -t ${DOCKER_REPOSITORY_NAMESPACE}/rocketmq:4.7.0 .
echo "done"
